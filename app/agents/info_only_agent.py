...
class CatalogAPI(Protocol):
    def get_non_empty_categories(self) -> List[str]: ...
...
async def handle_message(chat_id: str, text: str, api: CatalogAPI, telegram: TelegramSender):
    """
    Info-only UI:
      - Non-empty categories (6/page)
      - Items 5 per page, 'Show more'
      - No booking/delivery flows
      - Promo line only if present
      - HTML rendering for links; t.me → 'Open in Telegram'
    """
    intent = recognize_intent(text)

    # Add Service entry (router wizard handles the real flow)
    if intent["type"] == "add_service":
        return {"handled": True, "output": "Okay, let’s add a service. Send JSON or follow the prompts."}

    # Neutral reply for booking-like phrases
    ...
    # Browse specific category (via word mapping)
    if intent["type"] == "browse_category":
        cat = intent.get("category") or ""
        items = api.list_services_by_category(cat)
        if not items:
            return {"handled": True, "output": "Nothing available right now."}
